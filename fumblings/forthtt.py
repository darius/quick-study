...
def nativize(tt, pc, code):
    ...
    def retrace(pc, margin, tt):
        while True:
            insn = code[pc]
            pc += 1
            if insn == prim['halt']:
                untraceable[0] = True # XXX raise an exception?
                return
            elif insn == prim['(:)']:
                retrace(code[pc], margin, tt)
                pc += 1
            elif insn == prim[';']:
                return
            elif insn == prim['(if)']:
                z = pop()
                # XXX need to use margin with lines.append
                lines.append('if %s != 0:' % z)
                true_tt = tt.on_true()
                if true_tt is None:
                    genpush(margin + '    ')
                    lines.append(margin + ('    return %d' % (pc + 1)))
                    # return      # XXX
                else:
                    retrace(pc + 1, margin + '    ', true_tt)
                lines.append('else:')
                false_tt = tt.on_false()
                if false_tt is None:
                    genpush(margin + '    ')
                    lines.append(margin + ('    return %d' % code[pc]))
                else:
                    retrace(code[pc], margin + '    ', false_tt)
    ...
    retrace(pc, margin, tt)
    genpush('')                 # XXX
    lines.append('return %d' % pc) # XXX
    return prefix + lines
